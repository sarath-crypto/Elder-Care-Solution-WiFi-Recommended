...
bool load_config(ipc *ip){
	...
	for(unsigned int  i = 0;i < split.size();i++){
		string index = split[i].substr(0,split[i].find(" "));
		if(!index.compare("card")){
			int sp = split[i].find(" ");
			int ep = split[i].find(":");
			string index = split[i].substr(sp+1,ep-sp-1);
			sp = split[i].find("[");
			ep = split[i].find("]");
			string mic = split[i].substr(sp+1,ep-sp-1);
	...
	if(!stoi(cmd)){
		cmd = "insert into cfg (mouse_level, mouse_name, beacon_timeout,dir_max,camera,micro,voice_threshold,voice_start,voice_duration,sec,access) values(0,'DEAFULT',15,2,'DEFAULT','DEFAULT',100,0,0,'seckey','admin')";
		access_dbase(cmd,ip,DBNONE);
	}
...
void *dbproc(void *p){
	...
	unsigned long long prev_ts = 0;
		...
		if(id == 1){
			...
			cmd = "update nbuf set id=0";
			access_dbase(cmd,ip,DBNONE);
			syslog(LOG_NOTICE,"ecsysapp system configuration changed by user and system will reboot in 30 seconds");
			sighandler(1);
		}	
...
void *netproc(void *p){
	...
	if(cmd.find("_ap") !=  string::npos){
		...
		for(unsigned int i = 0;i < con.size();i++)if(cmd.find("_ap") !=  string::npos)con.erase(con.begin()+i);						
	}	
	...
	while(!exit_netproc){
		...
		ip->wifi = pc->con;
		if((ip->rm > RESOURCE_TH) && ip->wifi && !reboot){
